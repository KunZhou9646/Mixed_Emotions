...
for seen_speaker in os.listdir('/data07/zhoukun/VCTK-Corpus/mel'):
    seen_speakers.append(seen_speaker)
seen_speakers.sort()
seen_speakers = seen_speakers[:99]

for speaker in seen_speakers:
    files = [os.path.join('/data07/zhoukun/VCTK-Corpus/mel/%s'%speaker, fn) for fn in os.listdir('/data07/zhoukun/VCTK-Corpus/mel/%s'%speaker)]
...
f = open('evaluation_mel_list.txt','w')
...
f = open('training_mel_list.txt','w')
for index in range(len(file_list)):
    f.write(str(file_list[index]) + ' ' + str(num_frame_list[index]) + ' ' + str(num_phone_list[index]) + '\n')
f.close()
